...
Workspace: <workspace>/.yingwu/config.toml   (overrides global)
...
配置采用分层 TOML，优先级从低到高为：内置默认值 < 全局配置 < 工作区配置。
全局配置：<state-dir>/config.toml（参见 secrets.state_dir；路径格式遵循当前平台）。
工作区配置：<workspace>/.yingwu/config.toml（覆盖全局配置中的对应项）。
工作区命令放行规则只有在用户信任该工作区的精确规范化路径后才会生效。其他权限授予规则仅在全局范围内生效。
"""
...
class Config:
    ...
    # Yingwu Cloud (sign-in + managed connectors). Config, never constants:
...
def workspace_allowed_commands(workspace: str | Path) -> list[str]:
    ...
    path = Path(workspace).expanduser() / ".yingwu" / "config.toml"
...
def load_config(
    ...
    *,
    ...
    if workspace:
        w = Path(workspace).expanduser() / ".yingwu" / "config.toml"
        if w.is_file():
